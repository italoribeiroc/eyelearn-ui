...
- **Language selection**: a first-time visitor's language is auto-detected from their device's `Accept-Language` (`localeDetection: true` in `src/i18n/routing.ts`): any Portuguese variant (`pt`, `pt-BR`, `pt-PT`) gets `pt-BR`, everything else gets English. An explicit choice always wins and then sticks on every screen: `<LanguageSwitcher>` (or visiting any `/pt-BR/...` URL) sets the `NEXT_LOCALE` cookie, which next-intl checks before `Accept-Language`. Any new redirect or link built outside next-intl (route handlers, `window.location`, Stripe URLs) must carry the locale explicitly, like `localizedPath()` in `lib/billing/locale.ts` does.
